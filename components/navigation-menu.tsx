...
function NavigationMenuLinkWithMenu({ _title, href, sublinks }: HeaderLiksFragment) {
  ...
  return (
    <NavigationMenuItem key={`${href ?? ""}${_title}`} className="relative items-center gap-0.5">
      ...
      <NavigationMenuContent className="absolute top-[calc(100%+4px)] w-[clamp(180px,30vw,300px)] rounded-md border border-[--border] bg-[--surface-primary] p-0.5 dark:border-[#3a4f58] dark:bg-[--dark-surface-primary]">
          ...
          return (
            <ButtonLink
              ...
              className={clsx(
                "!px-3.5",
                cta.type === "primary" &&
                  "!bg-[#1A2B32] !text-white hover:!bg-[#2a3f48] dark:!bg-white dark:!text-[#1A2B32] dark:hover:!bg-gray-100"
              )}
              href={cta.href}
              intent={cta.type}
            >
  ...
  return (
    <>
      <button
        ...
        className="col-start-3 flex items-center justify-center gap-2 !justify-self-end rounded-sm border border-[--border] bg-[--surface-secondary] p-2 text-[--text-primary] dark:border-[#3a4f58] dark:bg-[#2e454e] dark:text-[--dark-text-primary] lg:hidden lg:h-7"
      ...
      <div className="block lg:hidden">
        {isOn ? (
          <>
            {/* Backdrop overlay - positioned below full header (navbar + banner) */}
            <div
              className="fixed inset-0 top-[var(--full-header-height)] z-[100] bg-black/50"
              onClick={handleOff}
            />
            {/* Menu content - positioned below full header */}
            <div className="fixed left-0 top-[var(--full-header-height)] z-[101] h-[calc(100vh-var(--full-header-height))] w-full overflow-y-auto border-t border-[--border] bg-[--surface-primary] dark:border-[#3a4f58] dark:bg-[#1A2B32]">
              ...
              <nav className="flex flex-col gap-4">
                {navbar.items.map((link) =>
                  link.sublinks.items.length > 0 ? (
                    ...
                    <Link
                      key={link._id}
                      className="flex items-center gap-2 rounded-sm px-3 py-1.5 text-[--text-primary] dark:text-[--dark-text-primary] hover:bg-[--surface-tertiary] dark:hover:bg-[#2e454e]"
                      href={link.href ?? "#"}
                      onClick={handleOff}
                    >
                      {link._title}
                ...
                {rightCtas.items.map((cta) => {
                  return (
                    <ButtonLink
                      ...
                      className={clsx(
                        cta.type === "primary"
                          ? "!bg-[#1A2B32] !text-white hover:!bg-[#2a3f48] dark:!bg-[#D2B4FA] dark:!text-[#1A2B32] dark:hover:!bg-[#c9a5f7]"
                          : "!border-[--border] !text-[--text-primary] dark:!border-[#3a4f58] dark:!text-[--dark-text-primary] dark:!bg-transparent dark:hover:!bg-[#2e454e]"
                      )}
                    >
                      {cta.label}
                    </ButtonLink>
...
function ItemWithSublinks({
  ...
  return (
    <div key={_id}>
      <button className="flex items-center gap-2 px-3 py-1.5 text-[--text-primary] dark:text-[--dark-text-primary] hover:bg-[--surface-tertiary] dark:hover:bg-[#2e454e] rounded-sm" onClick={handleToggle}>
        {_title}
        <ChevronDownIcon
          className={clsx(
            "h-min transform text-[--text-tertiary] transition-transform dark:text-[#7a9aa8]",
      ...
      >
        {sublinks.map((sublink) => {
          ...
          return (
            <li key={sublink._id}>
              <Link
                className="flex items-center gap-2 rounded-md px-3 py-1.5 text-[--text-tertiary] dark:text-[#7a9aa8] hover:text-[--text-primary] dark:hover:text-[--dark-text-primary] hover:bg-[--surface-tertiary] dark:hover:bg-[#2e454e]"
                href={href}
                onClick={onClick}
              >
                {_title}
