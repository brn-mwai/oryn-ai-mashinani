...
import { OrynHomePage } from "../_components/oryn-home";
...
export const generateMetadata = async ({
  ...
  // Use Oryn fallback metadata
  if (!process.env.BASEHUB_TOKEN) {
    return {
      title: orynContent.metadata.title,
      description: orynContent.metadata.description,
    };
  }
...
export default async function DynamicPage({
  ...
  // Use Oryn fallback for home page when BaseHub isn't configured
  if (!process.env.BASEHUB_TOKEN) {
    // Only show Oryn home page for root path
    if (!slugs || slugs.length === 0) {
      return <OrynHomePage />;
    }
    // For other paths without BaseHub, show 404
    notFound();
  }
  ...
  try {
    ...
    if (!page) {
      // Fallback to Oryn home page for root path
      if (!slugs || slugs.length === 0) {
        return <OrynHomePage />;
      }
      notFound();
    ...
    // Fallback to Oryn home page on error
    if (!slugs || slugs.length === 0) {
      return <OrynHomePage />;
    }
    notFound();
